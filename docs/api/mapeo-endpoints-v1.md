...
Rutas relativas al prefijo `/api/v1`. **Estado al 2026-09-22 (tras la primera tanda de alineación): 39 de las 45 operaciones del documento ya coinciden en ruta, verbo e `operationId`**; la columna «Acción» dice qué falta en cada una. La prueba `apps/api/tests/api/test_contract_conformance.py` lo verifica en cada ejecución.

## Fase 1 — Alta / MVP

| # | Documento | Hoy en la API | Acción |
|---|---|---|---|
| 1 | `POST /pieces` · `createPiece` | `POST /pieces` (stub) | Pendiente: esquema `PieceCreate` (tarea 4.1) |
| 2 | `GET /pieces` · `listPieces` (`page`, `limit`) | `GET /pieces` (implementado, cursor + filtros) | Pendiente: paginación `page`/`limit` (tarea 4.1) |
| 3 | `GET /pieces/{id}` · `getPieceById` | `GET /pieces/{piece_id}` (implementado) | Hecho: `getPieceById` |
| 4 | `PUT /pieces/{id}` · `updatePiece` | `PATCH /pieces/{piece_id}` (stub) | Hecho: `PUT /pieces/{id}` (`updatePiece`) |
| 5 | `DELETE /pieces/{id}` · `softDeletePiece` | `DELETE /pieces/{piece_id}` (stub) | Hecho: `softDeletePiece` |
| 6 | `GET /collections` · `listCollections` | `GET /collections` (implementado) | Pendiente: arreglo plano `CollectionItem` (tarea 4.2) |
| 7 | `POST /collections` · `createCollection` | `POST /collections` (implementado) | Hecho: `createCollection` |
| 8 | `GET /categories` · `listCategories` | `GET /vocabularies/categoria/terms` | Hecho: `GET /categories` sobre el vocabulario CATEGORY |
| 9 | `POST /categories` · `createCategory` | `POST /vocabularies/{code}/terms` | Hecho: `POST /categories` |
| 10 | `GET /conservation-states` · `listConservationStates` | `GET /vocabularies/estado-conservacion/terms` | Hecho: `GET /conservation-states` |
| 11 | `GET /locations/tree` · `getLocationsTree` | `GET /locations` (plano, implementado) | Hecho: `GET /locations/tree` con `LocationNode` |
| 12 | `POST /pieces/{id}/move` · `movePiece` | `POST /pieces/{piece_id}/movements` (stub) | Hecho: `POST /pieces/{id}/move` |
| 13 | `GET /pieces/{id}/location-history` · `getPieceLocationHistory` | `GET /pieces/{piece_id}/movements` (implementado) | Hecho: `GET /pieces/{id}/location-history` |
| 14 | `POST /imports/upload` · `uploadImportBatch` | `POST /imports` (stub) | Hecho: `POST /imports/upload` |
| 15 | `GET /imports/{batch_id}/diffs` · `getImportBatchDiffs` | `GET /imports/{batch_id}/preview` (stub) | Hecho: `GET /imports/{batch_id}/diffs` |
| 16 | `POST /imports/{batch_id}/confirm` · `confirmImportBatch` | `POST /imports/{batch_id}/approve` (stub) | Hecho: `POST /imports/{batch_id}/confirm` |
| 17 | `GET /search` · `searchPieces` | `GET /search` (implementado) | Parcial: `searchPieces` renombrado; faltan `collection_code`, `tenure_regime` y `page`/`limit` |
| 18 | `POST /media/upload` · `uploadMediaAsset` | `POST /pieces/{piece_id}/media` + `.../media/upload-url` (stubs) | Hecho: `POST /media/upload` (stub) |
| 19 | `POST /auth/login` · `login` | `POST /auth/login` (stub) | Pendiente: cuerpo `username`/`password` (tarea 4.7) |
| 20 | `GET /auth/me` · `getMe` | `GET /auth/me` (implementado) | Pendiente: rol único en la respuesta (tarea 4.7) |

## Fase 2 — Media

| # | Documento | Hoy en la API | Acción |
|---|---|---|---|
| 21 | `POST /pieces/{id}/identifiers` · `addPieceIdentifier` | igual (stub) | Hecho: ruta ya conforme |
| 22 | `DELETE /pieces/{id}/identifiers/{identifier_id}` · `deletePieceIdentifier` | no existe (hay `.../correction`) | Hecho como baja lógica (stub) |
| 23 | `GET /pieces/{id}/children` · `getPieceChildren` | no existe (se resuelve con filtro) | Hecho: `GET /pieces/{id}/children` |
| 24 | `POST /pieces/{id}/children` · `addPieceChild` | no existe | Hecho: `POST /pieces/{id}/children` (stub) |
| 25 | `POST /locations` · `createLocation` | igual (stub) | Hecho: ruta ya conforme |
| 26 | `PUT /locations/{id}` · `updateLocation` | `PATCH /locations/{location_id}` (stub) | Hecho: `PUT /locations/{id}` |
| 27 | `GET /locations/{id}/pieces` · `getPiecesInLocation` | no existe (filtro en `/pieces`) | Hecho: `GET /locations/{id}/pieces` (stub) |
| 28 | `GET /imports` · `listImportBatches` | igual (stub) | Hecho: `listImportBatches` |
| 29 | `POST /imports/{batch_id}/rollback` · `rollbackImportBatch` | `POST /imports/{batch_id}/revert` (stub) | Hecho: `POST /imports/{batch_id}/rollback` |
| 30 | `POST /reports/export-excel` · `exportExcelReport` | `POST /search/export` (stub) | Hecho: `POST /reports/export-excel` |
| 31 | `GET /reports/piece-card/{id}/pdf` · `exportPiecePdf` | no existe | Hecho: `GET /reports/piece-card/{id}/pdf` (stub) |
| 32 | `GET /reports/dashboard-stats` · `getDashboardStats` | `GET /quality/kpis` (stub) | Hecho: `GET /reports/dashboard-stats` |
| 33 | `GET /users` · `listUsers` | igual (stub) | Hecho: ruta ya conforme |
| 34 | `POST /users` · `createUser` | igual (stub) | Hecho: ruta ya conforme |
| 35 | `PUT /users/{id}/role` · `updateUserRole` | `PATCH /users/{user_id}` (stub) | Hecho: `PUT /users/{id}/role` (stub) |
| 36 | `GET /audit-logs` · `getAuditLogs` | `GET /audit` (implementado) | Hecho: `GET /audit-logs` |
| 37 | `POST /ai/suggest-cataloging` · `suggestCataloging` | `POST /ai/suggestions` (stub) | Hecho: `POST /ai/suggest-cataloging` |
| 38 | `POST /ai/validate-data` · `validateDataQuality` | `GET /quality/incomplete` + `/quality/kpis` (stubs) | Hecho: `POST /ai/validate-data` (stub) |
...
| 45 | `POST /ai/batch-enrich` · `batchAiEnrichment` | no existe (hay lote en `ia-sugerencia-terminos`) | Pendiente: requiere proponer el change de enriquecimiento por lote |
