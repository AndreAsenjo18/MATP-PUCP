...
- **Afecta**: `apps/api/app/api/`, `apps/api/app/core/` (errores HTTP, identidad provisional), `apps/api/app/modules/*/schemas.py` y routers, `apps/api/app/openapi_export.py`, `services/ai/matp_ai/providers/`, `services/ai/matp_ai/main.py`, `docs/api/`, `apps/web/src/lib/api/`, `package.json` (scripts `openapi`, `openapi:client`), CI, ADR-005.
