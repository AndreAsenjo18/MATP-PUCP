...
1. **Sin librería de componentes ni de formularios todavía.** _(Sustituido en cuanto a componentes por [ADR-012](ADR-012-sistema-diseno-frontend.md), 2026-09-23: sistema de diseño propio en `components/ui`, `domain` y `layout`. Lo relativo a formularios sigue vigente.)_ Tailwind 4 (ya instalado) + componentes propios pequeños (`ConfirmButton`, `Badges`, `AppShell`) y el elemento nativo `<dialog>` para confirmaciones. Los dos formularios con validación (login, editor de pieza) usan `useState` simple; no se instaló React Hook Form + Zod porque, con dos formularios cortos y sin pantallas adicionales previstas en esta fase, habría sido una dependencia nueva sin beneficio claro todavía.
