...
- **Afecta**: `deploy/vm/` (nuevo), `deploy/free-tier/` (nuevo), `scripts/deploy.sh`, `scripts/backup/` (nuevos), `.github/workflows/release.yml` (nuevo), `apps/*/Dockerfile` y `services/ai/Dockerfile` (usuario no root), `.env.example` (sección producción), `docs/despliegue/` (nuevo), `package.json` (scripts `backup:local`, `restore:test`).
