...
- **Primera tanda de alineación ya aplicada** (mismo día): `operationId` en camelCase, 12 rutas renombradas y 11 rutas nuevas del contrato. **39 de las 45 operaciones del documento ya coinciden** en ruta, verbo e `operationId`. Faltan las 3 de préstamos, la descarga masiva y el enriquecimiento por lote (necesitan que se propongan sus changes de backlog) y `/public/catalog` (bloqueada por C2). Sin tocar todavía el modelo de datos ni los esquemas: `PieceCreate`, la paginación `page`/`limit` y el rol único siguen pendientes (tareas 2, 4.1, 4.2 y 4.7).
- **Pendiente de decisión antes de seguir con los esquemas y el modelo**: C1 (numeración de requisitos, choca con la trazabilidad de las 12 specs y los 15 changes), C2 (`/public/catalog` sin autenticación frente a «solo uso interno»), C4 (modelo de datos: `code_i` en la pieza, categorías y estados como tablas, rol único) e I5 (¿se eliminan las 62 operaciones añadidas?).
